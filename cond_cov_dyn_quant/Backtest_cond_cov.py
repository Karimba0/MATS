...
# From https://github.com/BayerSe/VaR-Backtesting
class Backtest:
...
if __name__ == "__main__":
    ...
    if lstm:
        ...
        quantiles = [.01, .025, .05, .1, .01, .025, .05, .1]
        up_str = ('var_n_99', 'var_n_975', 'var_n_95', 'var_n_9', 'var_t_99', 'var_t_975', 'var_t_95', 'var_t_9')
        for a, b in zip(low_str, quantiles):
            print(a)
            print(b)
            Y = pd.read_csv(csv_name, delimiter=',')[['logreturns']].iloc[:, 0]
            X = pd.read_csv(csv_name, delimiter=',')
            bt = Backtest(actual=Y, forecast=X.loc[:, a], alpha=b)
            dq = bt.dq_bt()
            ct = bt.lr_bt()
            print(dq)
            print(ct)

        for a, b in zip(up_str, quantiles):
            print(a)
            print(b)
            Y = pd.read_csv(csv_name, delimiter=',')[['logreturns']].iloc[:, 0]
            X = pd.read_csv(csv_name, delimiter=',')
        ...
        Y = pd.read_csv('VaR_0.01.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.01.csv', delimiter=',')
        bt = Backtest(actual=Y, forecast=X.loc[:, "VARv"], alpha=0.01)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        print(ct)

        Y = pd.read_csv('VaR_0.025.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.025.csv', delimiter=',')
        bt = Backtest(actual=Y, forecast=X.loc[:, "VARv"], alpha=0.025)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        print(ct)

        Y = pd.read_csv('VaR_0.05.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.05.csv', delimiter=',')
        bt = Backtest(actual=Y, forecast=X.loc[:, "VARv"], alpha=0.05)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        print(ct)

        Y = pd.read_csv('VaR_0.09.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.09.csv', delimiter=',')
        bt = Backtest(actual=Y, forecast=X.loc[:, "VARv"], alpha=0.09)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        print(ct)

        Y = pd.read_csv('VaR_0.91.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.91.csv', delimiter=',')
        bt = Backtest(actual=-Y, forecast=-X.loc[:, "VARv"], alpha=0.09)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        ...
        Y = pd.read_csv('VaR_0.95.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.95.csv', delimiter=',')
        bt = Backtest(actual=-Y, forecast=-X.loc[:, "VARv"], alpha=0.05)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        ...
        Y = pd.read_csv('VaR_0.975.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.975.csv', delimiter=',')
        bt = Backtest(actual=-Y, forecast=-X.loc[:, "VARv"], alpha=0.025)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
        ...
        Y = pd.read_csv('VaR_0.99.csv', delimiter=',')[['logreturnsv']].iloc[:, 0]
        X = pd.read_csv('VaR_0.99.csv', delimiter=',')
        bt = Backtest(actual=-Y, forecast=-X.loc[:, "VARv"], alpha=0.01)
        dq = bt.dq_bt()
        ct = bt.lr_bt()
        print(dq)
