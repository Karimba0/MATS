...
dataset_test, dataset_test_x = fu.batch_dataset_test(test_x, test_y, par.reservoir, par.n_steps, par.ts_out_length, batch_size=1,
                                     drop_remainder=True, n_prefetch_batch=1, map_fn=False, n_map_threads=None,
                                     shuffle=True, shuffle_buffer_size=None, repeat=1)
...
D = keras.models.load_model('C' + '.h5')
D.load_weights('C' + '.weights.h5')
...
if par.optimizer == 'adam':
    for ep in tqdm.trange(epochs, desc='Epoch Loop'):
        ...
        for tr_x, tr_x_D, tr_y in tqdm.tqdm(dataset_train, desc='Inner Epoch Loop', total=len_dataset):
            ...
            # sample
            if G_optimizer.iterations.numpy() % 95 == 0:
                x_fake = sample(tr_x)
                print(x_fake)
...
# print out to see if model converged
print(pred)
print(te_y)
print(np.subtract(pred, tf.transpose(te_y[0, :, :]))/np.mean(te_y))
G.save('G' + ".h5")
